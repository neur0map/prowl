...
// BaseURL is the Hyper proxy root, overridable for a staging box exactly as
// the harness overrides it.
var baseURL = sync.OnceValue(func() string {
	if e := os.Getenv("HYPER_URL"); e != "" {
		return e
	}
	return "https://hyper.charm.land"
})

// BaseURL returns the proxy's root URL.
...
func pollOnce(ctx context.Context, deviceCode string) (TokenResponse, error) {
	...
	url := fmt.Sprintf("%s/device/auth/%s", BaseURL(), deviceCode)
