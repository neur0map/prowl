...
func ValidateMetamorphicTriples(cases []Case, triples []MetamorphicTriple) error {
	...
	seenTriples, usedCases := map[string]bool{}, map[string]string{}
	for _, triple := range triples {
		...
		for _, id := range ids {
			...
			expectedTruthID := tripleGroundTruthID(id, triple.GroundTruthID)
			found := false
			for _, truth := range c.GroundTruth {
				if truth.ID == expectedTruthID {
					found = true
				}
			}
			if !found {
				return fmt.Errorf("metamorphic case %s does not contain case-qualified defect %s", id, expectedTruthID)
			}
		}
...
func ScoreMetamorphic(scores []TrialScore, triples []MetamorphicTriple) map[string]MetamorphicMetric {
	...
	for _, condition := range []string{ConditionControl, ConditionTreatment} {
		...
		for _, triple := range triples {
			beginning := metamorphicHitRate(scores, condition, triple.BeginningCaseID, tripleGroundTruthID(triple.BeginningCaseID, triple.GroundTruthID))
			middle := metamorphicHitRate(scores, condition, triple.MiddleCaseID, tripleGroundTruthID(triple.MiddleCaseID, triple.GroundTruthID))
			end := metamorphicHitRate(scores, condition, triple.EndCaseID, tripleGroundTruthID(triple.EndCaseID, triple.GroundTruthID))
...
func tripleGroundTruthID(caseID, familyID string) string {
	return caseID + ":" + familyID
}
