...
func ValidateManifest(manifest Manifest) error {
	...
	seenGroundTruth := map[string]string{}
	validateSet := func(set string, cases []Case) error {
		for i := range cases {
			...
			for _, truth := range c.GroundTruth {
				...
				if prior := seenGroundTruth[truth.ID]; prior != "" {
					return fmt.Errorf("ground-truth id %q appears in cases %s and %s", truth.ID, prior, c.ID)
				}
				groundIDs[truth.ID] = true
				seenGroundTruth[truth.ID] = c.ID
