...
func TestValidateManifestRejectsGloballyDuplicateGroundTruthIDs(t *testing.T) {
	...
	second := Case{ID: "case-b", Repository: "repos/b", BaseSHA: strings.Repeat("c", 40), HeadSHA: strings.Repeat("d", 40), RawAdditions: 301, DefectClass: "local_logic", GroundTruth: []GroundTruth{{ID: "duplicate", DefectClass: "local_logic", Summary: "second defect", Scenario: "second failure", Locations: []Location{{Path: "b.go", StartLine: 1, EndLine: 1}}}}}
	...
	addTriple := func(set *[]Case, prefix, repository, base string) MetamorphicTriple {
		...
		for index, id := range ids {
			*set = append(*set, Case{
				...
				GroundTruth: []GroundTruth{{ID: tripleGroundTruthID(id, "bug"), DefectClass: "local_logic", Summary: "guard failure", Scenario: "bad input passes", Locations: []Location{{Path: "a.go", StartLine: 1, EndLine: 1}}}},
			})
		}
		return MetamorphicTriple{ID: prefix + "-triple", GroundTruthID: "bug", BeginningCaseID: ids[0], MiddleCaseID: ids[1], EndCaseID: ids[2]}
	}
