...
func TestBlindAdjudicationInputOmitsClientConditionAndHostIDs(t *testing.T) {
	...
	if got := input.GroundTruthCases["truth"]; len(got) != 1 || got[0] != "case" {
		t.Fatalf("ground-truth cases = %#v", got)
	}
	digest, err := jsonDigest(input)
	if err != nil {
		t.Fatal(err)
	}
	if err := ValidateAdjudication(Adjudication{Schema: AdjudicationSchema, CandidateDigest: digest, Frozen: true, Edges: []EligibilityEdge{{FindingID: "foreign", GroundTruthID: "truth"}}}, input); err == nil {
		t.Fatal("foreign adjudication edge accepted")
	}
	crossCase := input
	crossCase.GroundTruthCases = map[string][]string{"truth": {"other"}}
	crossDigest, _ := jsonDigest(crossCase)
	if err := ValidateAdjudication(Adjudication{Schema: AdjudicationSchema, CandidateDigest: crossDigest, Frozen: true, Edges: []EligibilityEdge{{FindingID: "canonical", GroundTruthID: "truth"}}}, crossCase); err == nil {
