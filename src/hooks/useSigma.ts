      ...
      nodeReducer: (node, data) => {
        ...
        if (animation) {
          ...
          if (animation.type === 'pulse') {
            ...
            res.zIndex = 5;
            res.highlighted = true;
          } else if (animation.type === 'watcher') {
            // Green pulsating glow for live file watcher activity
            // Slower oscillation: 2 full cycles over the animation duration
            const watcherPhase = (Math.sin(progress * Math.PI * 2) + 1) / 2;
            const sizeMultiplier = 1.6 + watcherPhase * 1.0;
            res.size = (data.size || 8) * sizeMultiplier;
            // Pulse between system green and bright green-white
            res.color = brightenColor('#30D158', 1 + watcherPhase * 0.6);
            res.zIndex = 6;
            res.highlighted = true;
          }
