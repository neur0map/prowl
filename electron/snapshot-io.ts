...
export class SnapshotIO {
   ...
   * Re-creates `.prowl/` directory before each step to guard against
   * races where the directory is removed between mkdir and write/rename.
   */
  async writeSnapshot(projectPath: string, data: Uint8Array): Promise<void> {
    ...
    // Write lock with PID
    try { await writeFile(lockPath, String(process.pid), 'utf-8') } catch { /* ok */ }

    try {
      await writeFile(tmpPath, Buffer.from(data))
      // Ensure directory still exists before rename (race guard)
      await mkdir(dir, { recursive: true })
      await rename(tmpPath, finalPath)
    } catch (err) {
      // Retry once: directory may have been recreated by another operation
